...
class UNetModelWrapper(UNetModel):
    def __init__(
        ...
        num_channels=128,
        num_res_blocks=2,
        channel_mult=(1, 2, 3, 4),  # Fully customizable channel multipliers
        learn_sigma=False,
        class_cond=False,
        num_classes=None,
        use_checkpoint=False,
        ...
        super().__init__(
            ...
            out_channels=(out_channels if not learn_sigma else dim[0] * 2),
...
class UNetModelWrapper2(UNetModel):
    def __init__(
            self,
            dim,
            num_channels=128,
            ...
            use_fp16=False,
            use_new_attention_order=False,
            conditioning_channels=3  # Explicitly define conditioning channels
    ):
        """Dim (tuple): (C, H, W)"""
        image_size = dim[-1]
        attention_ds = [image_size // int(res) for res in attention_resolutions.split(",")]
        ...
        super().__init__(
            image_size=image_size,
            in_channels=dim[0] + conditioning_channels,  # Add conditioning channels to input
            model_channels=num_channels,
            out_channels=(out_channels if not learn_sigma else dim[0] * 2),
            num_res_blocks=num_res_blocks,
            attention_resolutions=tuple(attention_ds),
    ...
    def forward(self, t, x, conditioning=None, *args, **kwargs):
        """Forward pass with optional conditioning."""
        if conditioning is not None:
            # Ensure conditioning is the same spatial size as x
            if conditioning.shape[-2:] != x.shape[-2:]:
                raise ValueError(
                    f"Conditioning shape {conditioning.shape[-2:]} does not match input shape {x.shape[-2:]}")

            # Concatenate conditioning information along channel dimension
            x = th.cat([x, conditioning], dim=1)  # (B, 12, H, W) expected
